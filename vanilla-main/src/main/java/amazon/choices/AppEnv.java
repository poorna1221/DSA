...
public enum AppEnv {
	DEVELOP("app.env.develop"), STAGING("app.env.staging");
	...
	public static AppEnv parse(String label) {
		if (BY_LABEL.get(label) == null) {
			throw new IllegalStateException(String.format("%s is not a valid app env choice. Pick your app env from %s."
					+ "Check the value of 'APP_ENV' property in amazon.choices.conf; Or in CI, if running from continuous integration.",
